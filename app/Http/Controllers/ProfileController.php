...
class ProfileController extends Controller{
  ...
  public function CheckFriend($idA,$idB){
    $a_friend_b = Users_make_friends::where([['id_user','=',$idA],['id_request_user','=',$idB]])->value('status');
    $b_friend_a = Users_make_friends::where([['id_user','=',$idB],['id_request_user','=',$idA]])->value('status');

    if(($a_friend_b == '0') || ($b_friend_a == '0') || ($a_friend_b == '1') || ($b_friend_a == '1')){
      //sono sono presenti nei record amicizia
      if(($a_friend_b == '0') || ($b_friend_a == '0')){
  ...
  public function ShowUser($id){
    try{
      if($this->verify_cookie()){
        ...
        if($user->id_user != $logged_user->id_user && $user->profiloPubblico == 1 && ($check_friend == 'no_add' || $check_friend == 'wait_add')){
          //sono nel profilo di un altro utente non mio amico con profilo privato
          $case = 'profile_priv_notfriend';
        }
        if($user->id_user != $logged_user->id_user && $user->profiloPubblico == 0 && ($check_friend == 'no_add' || $check_friend == 'wait_add')){
          //sono nel profilo di un altro utente non mio amico con profilo pubblico
          $case = 'profile_pub_notfriend';
        }
        if($user->id_user != $logged_user->id_user && ($user->profiloPubblico == 0 || $user->profiloPubblico == 1) && $check_friend == 'added' ){
